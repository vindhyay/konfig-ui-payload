...
//		     echo "Running ${VERSION} on ${env.JENKINS_URL}"
            sh 'docker build --force-rm=true -t ${REPO}/${NAME}:pc-${BUILD_NUMBER} .'
         }
      }
       stage('Docker Image Testing'){
         steps {
          echo"unit testing"
          sh'docker image inspect ${REPO}/${NAME}:pc-${BUILD_NUMBER}'
          sh 'docker run --rm -p 3700:8081 --detach ${REPO}/${NAME}:pc-${BUILD_NUMBER}'
          sleep(20)
      ...
      stage('Publish'){
         when {
    expression {
        return env.BRANCH_NAME == 'pre-cloud';
        }
    }
         steps {
            echo"Publishing the source files to remote registry"
            sh 'docker push ${REPO}/${NAME}:pc-${BUILD_NUMBER}'
         }
      }
		stage('Deploy to Pre-Cloud'){
			when {
			expression {
			return env.BRANCH_NAME == 'pre-cloud';
			}
        ...
        steps{
			...
			sh 'ssh root@10.10.5.24 "kubectl -n test set image deployments/${DNAME} ${NAME}=${REPO}/${NAME}:pc-${BUILD_NUMBER}"'
            sh 'ssh root@10.10.5.24 "kubectl -n test rollout restart deployment ${DNAME}"'

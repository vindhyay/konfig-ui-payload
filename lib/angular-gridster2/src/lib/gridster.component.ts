...
export class GridsterComponent implements OnInit, OnChanges, OnDestroy, GridsterComponentInterface {
  constructor(
    ...
    // this.renderer.listen(this.el, 'dragover', this.showItemPreview.bind(this));
    // this.renderer.listen(this.el, 'drop', this.removeItemPreview.bind(this));
  }
  ...
  removeItemPreview() {
    const previewContainer: any = document.querySelector('.grid-item-preview-overlay');
  ...
  showItemPreview(e) {
    if (this.movingItem) {
      if (!this.diffX || !this.diffY) {
        const allPreviewElements = Array.from(document.querySelectorAll('.gridster-preview'));
        const previewElement = allPreviewElements.find((element: any) => element.style.display === 'block');
        this.diffX = Math.abs(e.clientX - previewElement.getBoundingClientRect().left);
        this.diffY = Math.abs(e.clientY - previewElement.getBoundingClientRect().top);
      }
      // preview overlay start
      const previewContainer: any = document.querySelector('.grid-item-preview-overlay');
      previewContainer.style.display = 'flex';
      previewContainer.style.width = this.movingItem.cols * this.curColWidth + 'px';
      previewContainer.style.height = this.movingItem.rows * this.curRowHeight + 'px';
      previewContainer.style.transform = `translate(${e.clientX - this.diffX}px, ${e.clientY - this.diffY}px) scale(1)`;
      // preview overlay end
    }
  }
